...
class WorldStore {
    ...
    setLocationSyncMetadataBatch(entries = []) {
        ...
        this.db.exec('BEGIN IMMEDIATE');
        try {
            // Clear the affected sheet's row ids first: rows are renumbered on every
            // sheet rewrite, and updating one-by-one trips UNIQUE(external_sheet_id,
            // sheet_row_id) whenever a location moves to a row another one still holds.
            const sheetIds = new Set(entries
                .map((entry) => entry.metadata && entry.metadata.external_sheet_id)
                .filter(Boolean));
            sheetIds.forEach((sheetId) => {
                this.db.prepare('UPDATE locations SET sheet_row_id = NULL WHERE external_sheet_id = ?').run(sheetId);
            });
...
module.exports = { WorldStore, normalizeWorld, LOCATION_TYPES };
