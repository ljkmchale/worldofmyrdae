...
const { normalizeWorld, LOCATION_TYPES } = require('./world-store');

// The sheet shows display labels ("Small Settlement"); the database stores ids ("town").
const TYPE_LABEL_BY_ID = new Map(LOCATION_TYPES.map(([id, label]) => [id, label]));
const TYPE_ID_BY_TEXT = new Map(LOCATION_TYPES.flatMap(([id, label]) => [
    [id.toLowerCase(), id],
    [label.toLowerCase(), id]
]));

function typeIdToLabel(typeId) {
    return TYPE_LABEL_BY_ID.get(typeId) || typeId;
}

function typeTextToId(text) {
    return TYPE_ID_BY_TEXT.get(String(text || '').trim().toLowerCase()) || '';
}
...
function locationToRow(realm, location, status = 'OK', message = '') {
    return [
        ...
        location.id || '',
        location.name || '',
        location.type ? typeIdToLabel(location.type) : '',
...
function parseSheetLocation(rowObject, rowNumber, seenKeys) {
    ...
    const rawType = String(rowObject.type || '').trim();
    const type = typeTextToId(rawType);
    ...
    if (!deleted && !rawType) issues.push('type is required');
    if (!deleted && rawType && !type) {
        issues.push(`type "${rawType}" is not recognized; use one of: ${LOCATION_TYPES.map(([, label]) => label).join(', ')}`);
    }
...
async function syncLocationsWithSheet(store, dataRoot, source = 'scheduled-sync') {
    ...
    parsedRows.forEach((row) => {
        if (row.issues.length) {
            ...
            if (row.id && dbEntries.has(row.key)) {
                // The database still has a valid copy: keep it and flag the row
                // instead of blocking the whole sync on the bad sheet edit.
                conflictCount += 1;
                conflictKeys.push(row.key);
                rowStatuses.set(row.key, { status: 'CONFLICT', message: `Sheet row was invalid (${message}); kept the database value.` });
                return;
            }
            invalidCount += 1;
            invalidKeys.push(row.key || `row:${row.rowNumber}`);
        ...
        if (!dbEntry && stateEntry) {
            ...
            // Deletion in the editor wins; the sheet edit is recoverable from the
            // spreadsheet's version history and the conflict is named in the status.
            conflictCount += 1;
            conflictKeys.push(row.key);
            rowStatuses.set(row.key, { status: 'CONFLICT', message: 'Location was deleted in the map editor and changed in the sheet; the deletion was kept.' });
            return;
        }
        ...
        if (dbChanged && sheetChanged) {
            conflictCount += 1;
            conflictKeys.push(row.key);
            rowStatuses.set(row.key, { status: 'CONFLICT', message: 'DB and sheet both changed since last sync. Kept the database value; the sheet edit was overwritten.' });
            return;
        }
        ...
        if (row.deleted) {
            if (!dbChanged) {
                ...
                conflictCount += 1;
                conflictKeys.push(row.key);
                rowStatuses.set(row.key, { status: 'CONFLICT', message: 'Sheet requested deletion, but DB changed since last sync. Kept the database value.' });
            }
            return;
        }
    ...
    const freshKeys = new Set(freshEntries.map(({ realm, location }) => makeRecordKey(realm, location.id)));
    ...
    }).concat(rejectedRows.filter((row) => !freshKeys.has(makeRecordKey(row[0], row[1]))));
    await writeRows(config, rows);
    const syncedAt = nowIso();
    if (typeof store.setLocationSyncMetadataBatch === 'function') {
    ...
    return updateStatus(store, dataRoot, {
        ...
        exportedRows: rows.length,
        conflictRowKeys: conflictKeys.slice(0, 25),
        invalidRowKeys: invalidKeys.slice(0, 25),
        warning: (conflictCount || invalidCount)
            ? `${conflictCount} conflict row(s) kept the database value and ${invalidCount} invalid row(s) were flagged. See the syncStatus/syncMessage columns in the Sheet.`
            : undefined
    });
}
